...
from . import context
from .exceptions import CircularDependencyError, MissingReferenceError
from .models import FeatureMap
from .parser import FeatureMapParser, resolve_graph_edges
from .unified_config import WorkflowsConfig, load_unified_config
from .workflows import expand_workflows


def _discover_workflows_config(
    feature_map_path: Path,
    config_path: Path | None = None,
) -> WorkflowsConfig | None:
    """Discover workflows config from various locations.
    ...
    if ctx_config and ctx_config.exists():
        unified = load_unified_config(ctx_config)
        return unified.workflows
    ...
    if dir_config.exists():
        unified = load_unified_config(dir_config)
        return unified.workflows

    # 4. Current directory
    cwd_config = Path("mouc_config.yaml")
    if cwd_config.exists():
        unified = load_unified_config(cwd_config)
        return unified.workflows
...
def load_feature_map(
    ...
    *,
    ...
    feature_map = parser.parse_file(path)
    ...
    if workflows_config is None:
        workflows_config = _discover_workflows_config(path, config_path)
    feature_map.entities = expand_workflows(feature_map.entities, workflows_config)

    # 4. Resolve edges again (for new phase entities)
    resolve_graph_edges(feature_map.entities)

    # 5. Validate references and check for cycles
    validate_feature_map(feature_map)

    return feature_map


def validate_feature_map(feature_map: FeatureMap) -> None:
    """Validate the feature map for reference integrity and cycles."""
